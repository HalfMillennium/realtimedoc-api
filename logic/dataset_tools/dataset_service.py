...
class DataSetService:
    ...
    def get_spending_context(self, query_text, dataset_id = 'us_consumer_spending') -> List[str]:
        self.logger.info(f"BLOOP: Querying spending context for dataset: {dataset_id}")
        ...
        try:
            ...
            self.logger.info(f"Initializing spending database: {dataset_id}_embeddings")
            ...
            collection = client.get_collection(name=f"{dataset_id}_embeddings")
            self.logger.info(f"SPENDING INIT RESULTS: {init_spending_db_result}; Collection: {collection}")
        ...
        self.logger.info(f"query_text: {query_text}, results: {results}")
        documents = []
        if len(results) != 0:
